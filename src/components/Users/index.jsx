...
const Users = () => {
    ...
    const[ infoRecords, setInfoRecord ] = useState({maxItems: 10, totalPages: 1, totalRecords: 0,teamOptions: [],filters:defaultFilters,search:""});
    ...
    useEffect(() => {
        if(params.get("msg") === '1') renderMsg("Usuário cadastrado com sucesso.","success");
        if(params.get("msg") === '2') renderMsg("Usuário(s) atualizado(s) com sucesso.","success");
        if(params.get("msg") === '3') renderMsg("Usuário(s) excluído(s) com sucesso.","success");
        if(params.get("msg") === '4') renderMsg("Erro no servidor. Tente novamente mais tarde.","danger");
        params.delete("msg");
        var data ={
        ...
        api.post('/users/pages', data).then((res)=>{
            ...
            setInfoRecord(v=>
                ({  ...v,
                    ...
                    teamOptions: res.data.teamOptions,
        ...
        <div className="panel-header">
            <div className="wrapper space-between">
                <div className="wrapper">
                    <Link to="/home"><u>Início</u> /</Link><h3> Equipe</h3>
                </div>
                <div className="wrapper">
                <button onClick={handleExcel} title="Excel" className="btn-custom btn-medium btn-border btn-pill"><i className="fa fa-cloud-arrow-down"></i>Exportar</button>
        ...
        <div className="panel-body">
            ...
            <div className="table-custom">
                <div className="wrapper space-between table-custom-info">
                    <div className="wrapper">
                        <Link to={`/users/edit?id=0&copy=${rowsSelected[0]}`} className={document.querySelectorAll(".selected").length === 1 ? "btn-icon warning" : "btn-icon disabled-border"}><i className="fa-solid fa-copy"></i></Link>
                        <button onClick={handleArchive} className={rowsSelected.length > 0 ? "btn-icon info" : "btn-icon disabled-border"}><i className="fa-solid fa-inbox"></i></button>
                        <button onClick={handleOpenMultiple}  className={rowsSelected.length > 0 ? "btn-icon warning" : "btn-icon disabled-border"}><i className="fa-solid fa-edit"></i></button>                                                     
                        <button className={rowsSelected.length > 0 ? "btn-icon danger" : "btn-icon  disabled-border"} onClick={handleRemove}><i className="fa-solid fa-trash"></i></button>
                        <h5 className="ml">{rowsSelected.length} selecionado(s)</h5>
                    </div>                                             
                    <div className="wrapper">
                        ...
                        <div className="dropdown">
                            <button 
                            className="btn-custom btn-border" 
                                ...
                                <div className="menu filters">
                                    <div className="menu-wrapper">
                                        ...
                                        <div className="menu-body">
                                            ...
                                            {
                                                infoRecords.teamOptions.map((team,i) => {
                                                    if(i>=2&&i<4)return <CheckBoxPill 
                                                    placeholder={team !== "" ? team : "Indefinido"}
                                                    css="info"
                                                    value={team}
                                                    onClick={handleFilter}
                        ...
                        <div className="dropdown">
                            <button onClick={handleOpenOrder} className="btn-custom btn-border"><i className="fa-solid fa-arrow-down-wide-short"></i> Ordem</button>
                            { openOrder &&
                        ...
                        {                                
                            (() => {
                                ...
                                for( var i = 1; i <= len; i++ ) {
                                    ...
                                    id={pages[i - 1]} 
                                    className={`btn-custom btn-rounded ${currentPage === pages[i - 1] ? "" : "secondary03"}`} 
